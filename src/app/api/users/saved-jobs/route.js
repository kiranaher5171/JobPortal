...
export async function GET(request) {
  try {
    ...
    // User API - only return user's own saved jobs
    ...
    let query = {
      $or: [
        ...
        { userEmail: { $regex: new RegExp(`^${normalizedUserEmail}$`, 'i') } }
      ]
    };

    // Fetch saved jobs with limit for better performance
    const limit = 500; // Reasonable limit for saved jobs
    ...
    // Filter out null values (deleted jobs) - already filtered above
    const validSavedJobs = savedJobsWithDetails;
